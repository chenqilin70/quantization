...
public class HBaseDaoImpl extends BaseDaoImpl implements HBaseDao{
    ...
    public <T> T scan(String tableName, Scan scan, HBaseExecutors.ScanExecutor<T> executor) {
        putData("testtable","161604"+new Random().nextInt(1000),"baseinfo","abc","val:scan"+new Random().nextInt(1000));
    ...
    public void scanForEach(String tableName,Scan scan,HBaseExecutors.ScanForEachExecutor executor){
        scan(tableName,scan,scanner->{
            putData("testtable","161604"+new Random().nextInt(1000),"baseinfo","abc","val:scanforeach"+new Random().nextInt(1000));
            while(true){
                Result next = scanner.next();
                if(next==null){
                    putData("testtable","161604"+new Random().nextInt(1000),"baseinfo","abc","val:break"+new Random().nextInt(1000));
                    break;
                }
